...
`exiftree.sh` builds a clean, chronological directory tree for photos and videos by reading their metadata and copying them into date-based folders. It scans a source directory for common image/video formats, extracts the best available capture timestamp, and copies each file into a `YYYY/MM/DD` folder under the destination with a `YYYYMMDD_HHMMSS`-based filename. When metadata is missing, it falls back to the file modification time. It also keeps a SHA-256 database to skip duplicates on subsequent runs.
...
- Creates destination paths like `DEST/YYYY/MM/DD/YYYYMMDD_HHMMSS.ext`.
...
```bash
./exiftree.sh /path/to/source /path/to/destination
```
